...
function Adicionar(ev) {
  ev.preventDefault();
  const btnAdd = document.getElementById ("btnModalAdd");
...
function Editar(idPessoa, nomeTabela, emailTabela, tipoNum, myModalEditar) {
  console.log('id pessoa', idPessoa);
  const urlEditar =
    document.getElementById("urlInput").value + "/pessoa/" + idPessoa;
  ...
  const btnEditar = document.getElementById("editarModal");
  ...
  btnEditar.onclick = function (event) {
    EditarPut(event, urlEditar, myModalEditar, btnEditar);
  };

  myModalEditar.show();
}

function EditarPut(ev, url, myModalEditar, btnEditar) {
  ev.preventDefault();
